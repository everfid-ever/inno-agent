...
type UserDomainSVC interface {
	// Login 通过 Synapse 完成登录，返回用户实体与 token
	// authType: phone-password / phone-verify / email-password / email-verify / studentId-password / studentId-verify
	Login(ctx context.Context, authType, authId, verify string) (*entity.User, string, bool, error)

	// Register 通过 Synapse 完成注册，返回 token
	Register(ctx context.Context, authType, authId, verify, password string) (string, error)
